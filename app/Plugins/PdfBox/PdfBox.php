...
class PdfBox implements PdfConverter
{
    ...
    protected function execute(Command $command)
    {
        ...
        $command_str =(string) $command . ' 2>&1';
        if($this->pass){
            //dd($command, $this->_options, (string) $command . ' 2>&41');
            $command_str = str_replace('ExtractText','ExtractText -password "'. $this->pass .'"',$command_str);
        }

        exec($command_str, $stdErr, $exitCode);
