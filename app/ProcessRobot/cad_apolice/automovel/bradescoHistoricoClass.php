 ...
 */
class bradescoHistoricoClass extends ProcessAutomovelClass{
    ...
    //***** obs: como esta classe não tem o bradescoInsurer, então deve conter os códigos abaixo para ficar compatível com o restante da programação ******
    /**
     * Configuração para o padrão do número do quiver
     */
    public function numQuiverConfig(){
        return [
    ...
    private function modelo01(){
        ...
        //dd($n);
        if(strpos($n,'Descrição do Seguro	FROTA')!==false){
        ...
        //Endereço de pernoite do veículo
            ...
            if($data['segurado_pernoite_cep_1']==''){
                $data['segurado_pernoite_cep_1'] = TextUtility::getSearchText($this->text,'RISCO','cep',['side'=>'right']);
            }
        ...
        $tmp = $this->getX1(['start'=>'Dados Básicos da Apólice','split'=>false]);
            ...
            if($data['fpgto_tipo']=='cartao'){
                ...
                $tmp = $this->getX1(['start'=>'Nº daPrestação','split'=>false,'end'=>'Informações do Sinistro','remove'=>['Nº daPrestação','Informações do Sinistro']]);
                $tmp = TextUtility::getPartOfStr($tmp,['start'=>'DataControle','remove'=>'DataControle']);
                //$tmp = str_replace(['1 PARCELADEBITO','19/02/2021',chr(9).chr(9)], ['00006516','',chr(9)], $tmp);
        ...
        $tmp=str_replace(['Endosso',',0 '],[' ',',00 '],$tmp);
        
        foreach(explode(chr(10),$tmp) as $line){
            $find=0;
            TextUtility::getSearchText($line,'',function($v,$left,$right) use(&$valores,&$datavenc,&$find){
                if(is_numeric($v) && (int)$v>=1 && $v<=12){
                    $v=explode(' ',$right)[0];
                    if(ValidateUtility::isDate($v) && $find==0){
                        $datavenc[]=$v;
        ...
        //dd(count($valores), $datavenc, $valores);
