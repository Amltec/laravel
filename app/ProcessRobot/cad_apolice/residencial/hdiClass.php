 ...
 */
class hdiClass extends ProcessResidencialClass{
    use hdiInsurer;
    ...
    */
    private function processTipo01(){
       ...
       // dd($blocktext);
        ...
        $bairro_seg = TextUtility::getPartOfStr($block_segurado, ['start'=>'bairro :','end'=>'cidade :','sanitize'=>true]);
       ...
       // dd($endereco_seg,$endereco,$numero_seg,$numero);
        if($endereco_seg==$endereco && $numero_seg==$numero){//verifica se o endereço do local segurado é o mesmo do segurado
            ...
            $blocktext1 = TextUtility::getPartOfStr($blocktext, ['start'=>$numero,'sanitize'=>true]);
            if($blocktext1==''){
                $blocktext1 = $blocktext;
            }
            //dd($blocktext,$numero);
            $cep      = TextUtility::getSearchText($blocktext1,'','cep',['side'=>'right']);
            $estado   = TextUtility::getSearchText($blocktext1,'- '.$cep,'value',['side'=>'left']);
            $cidade   = TextUtility::getSearchText($blocktext1,'- '.$estado,'value',['side'=>'left']);
            $bairro ='';
            $complemento = '';
        }

        $data['residencial_endereco_1'] = $this->formatField1($endereco);
        $data['residencial_numero_1'] = $this->formatField1($numero);
        if($complemento){
            $data['residencial_compl_1'] = $this->formatField1($complemento);
        }else{
             $data['residencial_compl_1'] = '';
        }

        if($data['residencial_compl_1']=='' && $blocktext1!=''){
            $complemento = TextUtility::getPartOfStr($blocktext1, ['start'=>'/ ','end'=>'-','sanitize'=>false]);
            $data['residencial_compl_1'] = str_replace(['/ ','-'], '', $complemento);
        }
        $data['residencial_bairro_1'] = $this->formatField1($bairro);
        $data['residencial_cidade_1'] = $this->formatField1($cidade);
        $data['residencial_uf_1'] = $this->formatField1($estado);
