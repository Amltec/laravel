...
class PgtoData----bkp{
    ...
    protected $validate_iof_margem=0.06;
        ...
        //**** número de prestações ****
            ...
            if($is_tbl_valores){//achou a tabela de valores
                ...
                //*** monta automaticamente a tabela das parcelas ***
                    ...
                    $dt_parcela_1 = ($opt['dt_parcela_1']??false) ? $opt['dt_parcela_1'] : $data['inicio_vigencia'];
                    $dt_parcela_2 = ($opt['dt_parcela_2']??false) ? $opt['dt_parcela_2'] : $data['inicio_vigencia'];
     ...
     */
    public static function validateAll(&$dados,$parcelas,$marg_parc=0.9,$marg_iof=0.05,$change_data=false){
        $total      = (float)FormatUtility::nDecimal($dados['fpgto_premio_total']);
        $liquido    = (float)FormatUtility::nDecimal($dados['fpgto_premio_liquido']);
        $custo      = (float)FormatUtility::nDecimal($dados['fpgto_custo']);
        $adicional  = (float)FormatUtility::nDecimal($dados['fpgto_adicional']);
        $servico    = (float)FormatUtility::nDecimal($dados['fpgto_premio_liq_serv']);
        $iof        = (float)FormatUtility::nDecimal($dados['fpgto_iof']);
        $juros      = (float)FormatUtility::nDecimal($dados['fpgto_juros']);
        $juros_md   = (float)FormatUtility::nDecimal($dados['fpgto_juros_md']);
        $desc       = (float)FormatUtility::nDecimal($dados['fpgto_desc']);
        ...
        if($marg_parc){
            ...
            foreach($parcelas as $p => $d){
                $sum+= FormatUtility::nDecimal($d['fpgto_valorparc']);
            }
            ...
            if($dados['fpgto_desc']){//existe desconto para comparar
                ...
                if(!self::checkPremioTotal($dados['fpgto_premio_total'],$sum2,$marg_parc)){//divergência do prêmio total em relação a soma das parcelas
                    ...
                    if(self::checkPremioTotal($dados['fpgto_premio_total'],$sum2,$marg_parc)){
                        ...
                        dd('Valor do prêmio total com desconto - em análise');
        ...
        if($marg_iof!==false){
            ...
            if(!ValidateUtility::isValueMargin($sum,$iof,$marg_iof,$marg_iof)){
                ...
                if(!ValidateUtility::isValueMargin($sum,$iof,$marg_iof,$marg_iof)){
                    ...
                    if(!ValidateUtility::isValueMargin($sum,$iof,$marg_iof,$marg_iof)){
                        if($dados['fpgto_desc']){//existe desconto para comparar (aqui verifica apenas considerando que o desconto já está no prêmio total)
                            $sum = (($liquido + $custo + $servico + $juros + $juros_md) - $desc) * self::$iof_perc;
                            $sum = round($sum,2);
                            if(!ValidateUtility::isValueMargin($sum,$iof,$marg_iof,$marg_iof)){
                                return ['success'=>false,'msg'=>'Divergência no valor (com desconto): Iof','code'=>'read13'];
                            }
                        }else{
        ...
        if(!self::checkPremioTotal($dados['fpgto_premio_total'],$sum,$marg_parc)){
            ...
            if($sum!=$dados['fpgto_premio_total']){
                if($dados['fpgto_desc']){//existe desconto para comparar (aqui verifica apenas considerando que o desconto já está no prêmio total)
                     $sum = $liquido + $custo + $servico + $iof + $juros + $juros_md - $desc;
                     $sum = FormatUtility::numberFormat($sum);
                     if($sum!=$dados['fpgto_premio_total']){
                        return ['success'=>false,'msg'=>'Divergência nos valores (com desconto): Prêmio Total + Líquido + Custo + Adicional + Serviços + Juros + Iof - Desconto','code'=>'read12'];
                     }
