...
trait azulInsurer{
    ...
    public function initInsurer(){
         $this->pdf_engine = 'ait_ocr01'; //nome do interpretador de pdf (valores em  \App\Utilities\FilesUtility::readPDF)
...
/**
    ...
    public function getDados(){
        ...
        if(!$n){
            $n=TextUtility::getPartOfStr($this->text,['start'=>'Vigência do Seguro:','end'=>'Processo ']);
    ...
    public function getPremio($data){
        ...
        if(empty($data['fpgto_tipo'])){
         ...
         $blocktext = TextUtility::getPartOfStr($this->text, ['start'=>'quantidade de itens:','end'=>'cobranca','sanitize'=>true]);
         //dd($blocktext);
