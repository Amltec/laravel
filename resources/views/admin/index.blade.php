...
@php
    ...
    $has_boleto = array_get($accountConfig,'seguradora_data.active')=='s' && array_get($accountConfig,'seguradora_data.active_boleto_seg')=='s'; //existe boleto ativado
    ...
    if(!function_exists('fnc_caixinhaStatus')){
        function fnc_caixinhaStatus($baselink,$i,$sx,$sopt){
            ...
            echo '<tr onclick="goToUrl(\''. $link .'\');" class="cursor-pointer" data-status="'.$sopt['st_ref'].'">
                ...
                <td class="nav-link col-today col-count info-number" data-sx="'. $sx .'" data-sref="'. $sopt['st_ref'] .'" onclick="goToUrl(\''. $link .'\'+last_date_process);window.event.stopPropagation();return false;">...</td>
                <td class="col-all col-count info-number" data-sx="'. $sx .'" data-sref="'. $sopt['st_ref'] .'">...</td>
            </tr>';
        }
    }
    ...
    echo view('super-admin.index-inc--process-errors',['regs'=>5,'is_title'=>true]);
    echo '<br><br><br>';
    ...
    echo '
    <div class="col-sm-5">
            <div class="box box-danger box-widget">
    ...
    if($has_boleto){
            ...
            if($boleto_list_err['boleto_seg'] && $boleto_list_err['boleto_quiver']){
                    $route = route('admin.app.get',['process_seguradora_data','boletos_list']);
                    ...
                    <div class="col-sm-5">
                        <div class="box box-primary box-widget">
                            ...
                            <div class="box-body no-padding">
                                ...
                                foreach($boleto_arr_status['boleto_seg'] as $s => $label){
                                    $c = $boleto_list_err['boleto_seg']['status'][$s];
                                    if($c==0)continue;
                                    $st_prefix = 'b_';
                                    echo '<tr onclick="goToUrl(\''.$route.'?status='. $st_prefix.$s .'\');" class="cursor-pointer"><td>'. $boleto_list_err['boleto_seg']['labels'][$s] .'</td><td class="col-count text-right fields-ctrl" data-code="'.$c.'"><span class="btn btn-'. ($s=='e' && $c?'danger':'none') .' btn-xs" data-st="'.$s.'">'. $c .'</span></td></tr>';
                                }

                                $st_prefix = 'q_';
                                $s = 'p,a,e';//pronto robô, andamento e erro
                                $tmp = $boleto_list_err['boleto_quiver']['status'];
                                $c = $tmp['p'] + $tmp['e'];
                                echo '<tr onclick="goToUrl(\''.$route.'?status='. $st_prefix.$s .'\');" class="cursor-pointer"><td>Pendente no Quiver</td><td class="col-count text-right fields-ctrl" data-code="'.$c.'"><span class="btn btn-none btn-xs" data-st="'.$s.'">'. $c .'</span></td></tr>';
...
.table-view1 tr[data-status="c"] .info-number span.is_val.fill{background:#dd4b39;color:#fff;}
.table-view1 tr[data-status="o"] .info-number span.is_val.fill, .table-view1 tr[data-status="0"] .info-number span.is_val.fill, .table-view1 tr[data-status="p"] .info-number span.is_val.fill,.table-view1 tr[data-status="a"] .info-number span.is_val.fill{background:#3c8dbc;color:#fff;}
...
function fncProcessCount(force,objLink){
    ...
    var _fx1=function(name,arr){
        for(var i in arr){
            var o=oTable.find('.col-'+name+'[data-sx="'+i+'"]');
                o.html( '<span class="is_val '+ (arr[i]?'fill':'zero') +'">'+ formatCurrency(arr[i],0) +'</span>' );
            if(name=='all' && i=='w'){//oculta se pendente de apólice (w) = 0
                if(arr[i]==0)o.closest('tr').hide();
            }
        };
    };
    ...
    var _fxSumGroup=function(oTable,list){//list = {code1:0,code2:0,...}
        ...
        var os=oTable.find('td[data-code]').each(function(){
            ...
            sum1+=n;
            td.html('<span class="btn btn-'+ (n?'danger':'none') +' btn-xs" data-st="c">'+ formatCurrency(n) +'</span>');
        });
        //soma todos
        var sum2=0;
        for(var i in list){sum2+=list[i];};
        var n=sum2-sum1;if(n<0)n=n*-1;
        os.filter('[data-code=_other]').html('<span class="btn btn-'+ (n?'danger':'none') +' btn-xs" data-st="c">'+ n + '</span>');
    };
};
fncProcessCount();
