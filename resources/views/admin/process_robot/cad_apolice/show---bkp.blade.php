...
if(!function_exists('__PCAShowItem01')){
    ...
    function __PCAShowItem01($blockname, $dataarr,$field,$field_to_form,$ctrlChanged_user,$ctrlChanged_robo,$dataPdf,$execsArr,$execsFinal,$validate_not,$fields_ignore_show,$i=null){
        ...
        foreach($dataarr as $col => $arr){
            ...
            //if($col=='exec_5' && $field=='veiculo_ci')dump([$col,$v,$is_change_quiver,$ctrlChanged_robo]);
            ...
            //if(substr($col,0,5)=='exec_' && $col=='exec_1' && $field=='fpgto_tipo_code')dump($field,$arr,$dataarr);
...
if($fileInfo['success']==false){
    ...
    $fileInfo['link_html']='<a href="'.$fileInfo['file_url'].'" target="_blank" title="Arquivo PDF que gerou este processo: '.$n.'">'. str_limit($n,20) .'</a>';
}else{
...
echo '<div class="row-max1"><div class="clearfix row">';
    echo '<div class="col-sm-3"><div class="box box-primary box-widget"><div class="box-body no-padding">
                <table class="table no-margin">
                    <tr title="Id '.$model->broker_id.'"><td>Corretora</td><td>
                        <span class="strong">';
                            if($model->broker){
                                if($user_logged_level=='user' || !$is_lock_form_edit){
    ...
    echo '<div class="col-sm-4"><div class="box box-primary box-widget"><div class="box-body no-padding">
        <table class="table no-margin">
            ...
            <tr><td>Segurado</td><td><span class="strong">'. ($segDados_db ? $segDados_db->segurado_nome : '-') .'</span></td></tr>
        </table>
    </div></div></div>';
    ...
    echo '<div class="col-sm-2"><div class="box box-primary box-widget"><div class="box-body text-center relative">
            <a class="btn btn-info strong" href="'.$fileInfo['file_url'].'" target="_blank">Acessar '. (VarsProcessRobot::$typesApolices[$data_type]??'none') .'</a>
...
echo '<div class="clearfix row row-max1" style="margin-top:10px;margin-bottom:10px;">';
    ...
    echo '<div class="col-sm-3">';
        echo '<span style="font-size:small;">Reprocessar<br></span>';
        ...
        echo '<a class="btn '.$model->status_color['bg'].'" title="'. $model->status_long_label .'" onclick="editOpen();return false;"><i class="fa fa-pencil"></i> '.$model->status_label.'</a>';
    echo '</div>';
...
//mensagem prinicipal de retorno **********    
        ...
        }else if(in_array($s,['0','o'])){//nestes status, não precisa exibir as mensagens de erro
            $msg[]=$model->status_long_label;
            $is_block_collapsed=true;
            
        }else if($dataPdf){//quer dizer que está indexado
            ...
            //captura a msg a partir do validate dos campos
                if($validate_not){
                    ...
                    if(isset($custom_codes_fields_edit[$model_data['error_msg']])){//é igual aos códigos de erros setados em $custom_codes_fields_edit
                        ...
                        $msg[]=$thisClass::getStatusCode($model_data['error_msg']);
                    
                    }else if(count($validate_not)==1 && $model_data['error_msg']!='read01'){//só tem uma msg de erro
                        $msg[]=$thisClass::getStatusCode($model_data['error_msg']) . '<br>'.$m;
                    }else{
    ...
    echo '<div id="dev-info-data" class="box box-primary box-widget hiddenx row-max1"><div class="box-body no-padding row">
            ...
            <div class="col-sm-6">
                <table class="table no-margin">
                    ...
                    <tr><td>Origem do Documento</td><td>'. ($model->process_auto ? 'Área de Seguradoras' : 'Manual') .'</td></tr>';
...
if($obs_admin || in_array($user_logged_level,['dev','superadmin'])){
    echo '<div id="dev-info-data" class="box box-primary box-widget row-max1">
            <div class="box-header with-border" onclick="shTableRows($(this),$(this).closest(\'#dev-info-data\'),\'auto\',\'.box-body\');">
                <h3 class="box-title">Observações</h3> <span class="fa icon-collapse fa-angle-down" style="font-size:14px;margin-left:5px;" data-icon="fa-angle-right|fa-angle-down"></span>'.
                ($user_logged_level=='dev' ? '<a href="#" class="btn btn-default btn-xs" id="btn_obs_admin" style="margin-left:10px;">Editar</a>' : '').'
            </div>
            <div class="box-body ">
...
if($is_lock_form_edit){
        ...
        echo view('templates.ui.form',[
            ...
            'content'=>function() use($model,$datablocks,$segDadosClass,$segParcelasClass,$segProdClass,$dataPdf,$segProd_title,$manual_form_dados,$manual_form_parcelas,$manual_form_prod,$is_user_manual_confirm){
                ...
                echo view('templates.components.checkbox',['name'=>'user_manual_confirm','list'=>['s'=>'Confirmo os dados preenchidos estão corretos'], 'value'=>($is_user_manual_confirm?'s':''), 'attr'=>'style="margin-left:10px;font-size:1em;"','class_item'=>'strong']);  //'class_group'=>($is_user_manual_confirm?'':'hiddenx'),
            }
        ]);
}
...
function fReprocess(force,xconfirm){//force=true
    if(xconfirm!==false){
...
//**** admin msg ****
@if($user_logged_level=='dev')
    $('#btn_obs_admin').on('click',function(e){
        e.preventDefault();
        awModal({
            ...
            html:function(oHtml){
                oHtml.html(
                    ...
                    '<div class="row">'+
                        '<div class="form-group col-sm-12">'+
                            '<div class="control-div">'+
                                '<textarea class="form-control" name="obs_admin" placeholder="" rows="10" style="resize: none;">'+ $('#obs_admin_textarea').html() +'</textarea>'+
                            '</div>'+
                        '</div>'+
                    '</div>'+
                    ''
            ...
            btClose:false,
            btSave:'Salvar',
            form:'method="POST" action="{{route($prefix.'.app.post',['process_cad_apolice','obs_admin'])}}" accept-charset="UTF-8" ',
            form_opt:{
